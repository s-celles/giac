...
#if defined GIAC_JULIA && defined __FreeBSD__
static const char * icas_gettext(const char * f){ return f; }
#else
static const char * icas_gettext(const char * f){ return gettext(f); }
#endif
...
bool is_help_text(const string &str,string &desc,string &usg,vector<string> &rel,vector<string> &ex) {
  string desc_text=icas_gettext("Description"),usg_text=icas_gettext("Usage"),rel_text=icas_gettext("Related"),ex_text=icas_gettext("Examples");
  string see_also=icas_gettext("See also");
...
void print_help_text(const string &desc,const string &usg,const vector<string> &rel,const vector<string> &ex) {
  ...
  bool newline=false;
  if (!desc.empty()) {
    printf("scheme:%s",string("(with \"font-series\" \"bold\" \"color\" \""+col+"\" \""+icas_gettext("Description")+":\")").c_str());
    texmacs_endbegin();
  ...
  if (!usg.empty()) {
    if (newline) printf("\n");
    texmacs_endbegin();
    printf("scheme:%s",string("(with \"font-series\" \"bold\" \"color\" \""+col+"\" \""+icas_gettext("Usage")+":\")").c_str());
    texmacs_endbegin();
  ...
  if (!rel.empty()) {
    if (newline) printf("\n");
    texmacs_endbegin();
    printf("scheme:%s",string("(with \"font-series\" \"bold\" \"color\" \""+col+"\" \""+icas_gettext("See also")+":\")").c_str());
    texmacs_endbegin();
  ...
  if (!ex.empty()) {
    ...
    if (newline) printf("\n");
    texmacs_endbegin();
    printf("scheme:%s",string("(with \"font-series\" \"bold\" \"color\" \""+col+"\" \""+icas_gettext("Examples")+":\")").c_str());
    int cnt=0;
...
bool system_status_ok(int status) {
  return status!=-1
#if !defined __MINGW_H && !defined VISUALC && defined HAVE_SYS_WAIT_H
...
void texmacs_output(const giac::gen & g,giac::gen & gg,bool reading_file,int no,const giac::context * contextptr){
  ...
  if (gg.type==giac::_STRNG) {
    ...
    if (is_help_text(*gg._STRNGptr,desc,usg,rel,ex)) {
      if (usg.find(icas_gettext("No help available for "))==0) {
        printf("%s, %s%s: %s?\n",usg.c_str(),icas_gettext("did you mean"),(rel.size()>1?icas_gettext(" one of"):""),join_strings(rel,", ").c_str());
      } else {
      ...
      string err_text=icas_gettext("Error");
...
void end_icas(){
#if defined HAVE_SIGNAL_H && !defined __MINGW_H
  if (giac::child_id>1){
...
int main(int ARGC, char *ARGV[]){
  ...
  bool inmcp=((ARGC>=2) && std::string(ARGV[1])=="--mcp");
  ...
  // Help and completion
  int helpitems;
  // export GIAC_DEBUG==-2 to output static_help*
  // export GIAC_DEBUG==-3 or -4 to output mcp tool list
#ifdef STATIC_BUILTIN_LEXER_FUNCTIONS
  if (giac::debug_infolevel<=-2 && giac::debug_infolevel>=-4){
#endif
  ...
  if (inemacs || inmcp){
	...
	prompt=true;
      ...
      char car;//,nxt;
#ifdef EMACS_DEBUG
      ofstream logfile(("log"+giac::print_INT_(k%16)).c_str());
#endif
      for (;;){
	int i=getchar();
	car=i;
        ...
        if (i!=-1){
          if (i<32)
            logfile << "Ctrl-" << giac::print_INT_(i) << '\n';
          else {
            logfile << car << '\n';
          }
          logfile.flush();
...
#ifdef HAVE_SIGNAL_H_OLD
	  if (inemacs && giac::messages_to_print.size()>1){
	    putchar(EMACS_DATA_BEGIN);
	    putchar(EMACS_ERROR);
  ...
  if (ARGC==1){
    ...
    cout << icas_gettext("Welcome to giac readline interface, version ") << GIAC_VERSION << '\n';
